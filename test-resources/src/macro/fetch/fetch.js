...
function test2() {
    ...
    req.text().then(function ff3(value) {
        TAJS_dumpValue(value);
        req.clone(); // should raise an error!
    }).catch(function ff4() {
...
function test3() {
    var r = new Response('foo=bar', {headers: {'content-type': 'application/x-www-form-urlencoded'}});
    r.formData().then(function ff6(value) {
        TAJS_dumpValue('Form data');
        TAJS_dumpValue(value);
    })
}
...
function test4() {
    ...
    r.json().then(function ff5(json) {
        TAJS_dumpValue('Executed print any JSON');
        TAJS_dumpValue(json);
        return new Blob();
    })
}
